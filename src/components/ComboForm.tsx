    ...
    return (
        <div className={'flex flex-col w-full h-full transition duration-600'}>
            <div className='w-full flex justify-between mt-4'>
                <h1 className='text-3xl font-bold mb-1 flex items-center'>Meats {selectedCombo ? <span className='text-base font-thin ml-2'>({selectedCombo.filter((i: any) => i.type === "meat").length} required)</span> : ""}</h1>
            ...
            <div className={selectedCombo?.filter((i: any) => i.type === "vegetable").length === 0 ? 'hidden' : ''}>
                <h1 className='text-3xl font-bold mt-2 mb-1 flex items-center'>Vegetables{selectedCombo ? <span className='text-base font-thin ml-2'>({selectedCombo.filter((i: any) => i.type === "vegetable").length} required)</span> : ""}</h1>
                <ol>
